...
const {
  changeAssetOwner,
} = require("../ethereum/contractExplorer/js/smartContracts");
const {
  mysqlPoolQueryB,
...
const checkAmountArray = async (randomList) => {
  ...
  await asyncForEach(randomList, async (amount, index) => {
    await checkAmountAfterMint(`000a${10 + index}@gmail.com`, amount).catch(
      (err) => {
        console.log(err);
      }
    );
...
const frontEndUserRegistry = async () => {
  ...
  describe("intergration testing of front-end user register", async function () {
    ...
    it("post user verify code ", async function () {
      user_verify_code = await mysqlPoolQueryB(
        "SELECT u_verify_code FROM user WHERE u_email = ?",
        [_email]
      );
...
const ForgetPassword = async () => {
  ...
  describe("Forget Password testing...", async function () {
    ...
    it("Verify the code sent in email", async function () {
      fp_verify_code = await mysqlPoolQueryB(
        "SELECT fp_verification_code FROM forget_pw WHERE fp_investor_email = ?",
        [email]
      );
...
const ForgetPassword2 = async () => {
  ...
  describe("Forget Password (KYC needed) API CICD test", async function () {
    ...
    it("apply for new password", async function () {
      await request
        ...
        .send({
          email: email,
          password: password,
          fp_imagef: "public/uploadImgs/1.jpg",
          fp_imageb: "public/uploadImgs/1.jpg",
          fp_bankAccountimage: "public/uploadImgs/1.jpg",
        })
        .set("Accept", "application/json")
        .expect(200)
        .then(async function (res) {
          await res.body.message.should.equal(
            "申請成功，請等待身份驗證通過後再以新密碼登入"
          );
        });
    });
    ...
    it("KYC approve by Platform Admin", async function () {
      ForgetPassword_application_time = await mysqlPoolQueryB(
        "SELECT fp_application_date FROM forget_pw WHERE fp_investor_email = ?",
        [_email]
      );
      ForgetPassword_application_time =
        ForgetPassword_application_time[0].fp_application_date;
      await request
    ...
    it("get order compliance", async function () {
      await request
        ...
        .query({
          ...
          tokenCount: amout,
          buyAmount: amout,
          userIdentity: "A128465975",
          fundCount: price * amout,
        })
        .set("Accept", "application/json")
        .expect(200)
        .then(async function (res) {
...
const FMS_PS_Approve_update = async (symbol, pe_columnName) => {
  describe("FMS+PS approved the update", async function () {
    ...
    it("FMS approved the update", async function () {
      currentTime = await mysqlPoolQueryB(
        "SELECT * FROM product_editHistory WHERE pe_symbol = ?",
        [symbol]
      );
      currentTime = currentTime[0].pe_applicationTime;
      console.log(currentTime);
      ...
      await request
        .get(
          `/product/ReviewUpdateProductAfterPublish/true/${symbol}/${pe_columnName}/${currentTime}/${RejectReason}`
        )
        .set("Cookie", token)
        .expect(302)
        .then
    ...
    it("Login FMS before do something", async function () {
      await request
        ...
        .set("Accept", "application/json")
        .send({
          m_id: "Platform_Supervisor",
          m_password: "Platform_Supervisor",
        })
        .expect(302)
        .then((res) => {
          res.header["set-cookie"].should.not.empty();
  ...
  describe("intergration testing of PS publish product", async function () {
    ...
    it("Add CSV Into DB By PS", async function () {
      await request
        ...
        .then(async function (res) {
          console.log(res.body);
          await res.body.messageForDeveloper.should.equal(
            "IncomeCSV文件寫入資料庫成功"
          );
        });
    });
    ...
    it("Deploy Crowdfunding By PS", async function () {
      await request
        ...
        .send({
          ...
          quantityMax: total,
          fundingGoal: goal,
          CFSD2: edit_product.p_CFSD,
          CFED2: edit_product.p_CFED,
        })
        .set("Cookie", token)
        .expect(200)
        .then(async function (res, err) {
...
const PSPauseProduct = async () => {
  describe("intergration testing of PS pause product", async function () {
    ...
    it("You need to wait 60 secs now", async function () {}).timeout(3000);
...
const makeOrderPaidAndWriteIntoCFC = async () => {
  describe("Make Order Paid And Write Into Crowdfunding", async function () {
    ...
    it("Make Order Paid", async function () {
      ...
      const result = await mysqlPoolQueryB(
        "UPDATE order_list SET ? WHERE o_bankvirtualaccount = ?",
        [sql, virtualAccount]
      ).catch(async (err) => {
        ...
        function (err, conn) {
          conn.createChannel(function (err, ch) {
            ...
            ch.publish(
              "timeserver",
              `addAssetbooksIntoCFC`,
              Buffer.from((getLocalTime() + 100).toString())
            );
            console.log(
              ` [x] Sent ${getLocalTime() + 100}_addAssetbooksIntoCFC`
            );
          });
          setTimeout(function () {
            conn.close();
  ...
  describe("intergration testing of PS mint token", async function () {
    ...
    it("Funding Close By PS", async function () {
      let result = await mysqlPoolQueryB(
        "SELECT * FROM product WHERE p_SYMBOL = ?",
        [symbol]
      );
      let data = result[0];
      await request
        ...
        .set("Cookie", token)
        .send({
          TimeOfDeployment: getLocalTime(),
          TimeTokenValid: parseInt(data.p_validdate),
          TimeTokenUnlock: parseInt(data.p_lockuptime),
          nftName: symbol,
          siteSizeInKW: data.p_size,
          maxTotalSupply: data.p_totalrelease,
          initialAssetPricing: data.p_pricing,
          pricingCurrency: data.p_currency,
          IRR20yrx100: parseInt(data.p_irr * 100),
          tokenURI: data.p_HCAT721uri,
          crowdFundingCtrtAddr: crowdFundingAddr,
        })
        .expect(200)
        .then(async function (res) {
    ...
    it("Mint Token By PS", async function () {
      await request
        .post(
          `/contracts/HCAT721_AssetTokenContract/${symbol}/mintSequentialPerContract`
        )
        .set("Cookie", token)
        .expect(200)
        .then(async function (res) {
    ...
    it("Wating a while(60s) for Minting...", async function () {}).timeout(
      1000
    );
...
const productinfo_api = (p_status) => {
...
const flow5 = async () => {
  describe("intergration testing of reaching funding goal but aborted", async function () {
    ...
    FMNUpdateProduct();
    await update_approve_flow();
    await getRandomList().then(async (result) => {
      console.log(result);
      await asyncForEach(result, async (amount, index) => {
        await frontEndUserOrdering(
          amount,
