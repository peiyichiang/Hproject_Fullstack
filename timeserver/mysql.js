...
const frontendPoolQuery = (a, b) => {
  ...
  const queryType = {
    ...
    remainRelease_Order: remainRelease_Order,
  };
...
const product = function () {
  ...
  var query1 = new Promise(async (resolve, reject) => {
    var queryStr = `SELECT p.p_state AS status,
                p.p_irr AS irr,
                pd.pd_icon AS icon,
                p.p_name AS name,
  ...
  var query2 = new Promise(async (resolve, reject) => {
    var queryStr = `SELECT p.p_SYMBOL AS symbol,
  ...
  var query5 = new Promise(async (resolve, reject) => {
    ...
    if (status == "AllProductForFrontend") {
      sql_extended =
        " OR p.p_state IN ('fundingClosed') OR p.p_state IN ('ONM') AND ia.ia_single_Forecasted_Annual_Income!=0";
      queryStr = queryStr + sql_extended;
    }
    const result = await mysqlPoolQueryB(queryStr, status).catch((err) => {
      console.log(err);
...
const asset = function () {
  ...
  var query1 = new Promise(async (resolve, reject) => {
    const queryStr = `SELECT new_table.name,new_table.location,new_table.symbol,new_table.PVOnGridDate,new_table.size,new_table.totalRelease,new_table.RPT,new_table.feedInTariff,new_table.forecastedPeriodIncomePerPiece,new_table.time, pd.pd_Image11
    ...
    const result = await mysqlPoolQueryB(queryStr, [date, user_email]).catch(
      (err) => {
        console.log(err);
        reject("[Error @ mysqlPoolQueryB]" + err);
      }
  ...
  var query2 = new Promise(async (resolve, reject) => {
    const queryStr = `SELECT p.p_SYMBOL AS symbol,
  ...
  var query4 = new Promise(async (resolve, reject) => {
    const queryStr = `SELECT p.p_SYMBOL AS symbol,p.p_totalrelease,p.p_feedintariff,
...
const queryOrder = function () {
  ...
  user_email = arguments[0][0];
  var query1 = new Promise(async (resolve, reject) => {
    const queryStr = `SELECT o.o_id AS id,
...
const addTxnInfoRow = (
  ...
  return new Promise(async (resolve, reject) => {
    const queryStr1 =
      "INSERT INTO transaction_info (t_txid, t_tokenSYMBOL, t_fromAssetbook, t_toAssetbook,  t_tokenId, t_txCount, t_holdingDays, t_txTime, t_balanceOffromassetbook) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    const result = await mysqlPoolQueryB(queryStr1, [
      txid,
      tokenSymbol,
      fromAssetbook,
      toAssetbook,
...
const deleteTxnInfoRows = (tokenSymbol) => {
  return new Promise(async (resolve, reject) => {
    ...
    const result = await mysqlPoolQueryB(queryStr1, [tokenSymbol]).catch(
      (err) => {
        reject("[Error @ mysqlPoolQueryB] " + err);
      }
    );
...
const addProductRow = async (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const sql = {
      ...
      p_PowerPlantEquipmentRegisteredLetter:
        powerPlantEquipmentRegisteredLetter,
...
const addSmartContractRow = async (
  tokenSymbol,
  addrCrowdFunding,
  addrHCAT721,
  maxTotalSupply,
...
const add3SmartContractsBySymbol = async (
  tokenSymbol,
  addrHCAT721,
  addrIncomeManager,
  addrTokenController
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "UPDATE smart_contracts SET sc_erc721Controller = ?, sc_erc721address = ?, sc_incomeManagementaddress = ? WHERE sc_symbol = ?";
    const result = await mysqlPoolQueryB(queryStr1, [
      addrTokenController,
      addrHCAT721,
      addrIncomeManager,
...
const deleteSmartContractRows = (tokenSymbol) => {
  return new Promise(async (resolve, reject) => {
    ...
    wlogger.debug(
      `deleteSmartContractRows result: ${JSON.stringify(result, null, 4)}`
    );
    resolve(true);
  });
};
  ...
  return new Promise(async (resolve, reject) => {
    ...
    bcrypt
      ...
      .then(async (hash) => {
        ...
        const result = await mysqlPoolQueryB(queryStr2, userNew).catch(
          (err) => {
...
const addUsersIntoDB = async (users) => {
  return new Promise(async (resolve, reject) => {
    ...
    const result = await asyncForEach(users, async (user, idx) => {
      ...
      wlogger.debug(
        `idx: ${idx}, email: ${email}, identityNumber: ${identityNumber}, eth_add: ${eth_add}, cellphone: ${cellphone}, name: ${name}, addrAssetbook: ${addrAssetBook}, investorLevel: ${investorLevel}, imagef: ${imagef}, imageb: ${imageb}, bank_booklet: ${bank_booklet}`
      );

      const result = await addUserRow(
        email,
        password,
        identityNumber,
        eth_add,
        ...
        reject(`addUserRow() failed. user: ${user}, err: ${err}`)
      );
...
const getAssetbookFromEmail = async (email) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "SELECT u_assetbookContractAddress FROM user WHERE u_email = ?";
...
const getAssetbookFromIdentityNumber = async (identityNumber) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "SELECT u_assetbookContractAddress FROM user WHERE u_identityNumber = ?";
    const result = await mysqlPoolQueryB(queryStr1, [identityNumber]).catch(
      (err) => {
...
Date.prototype.myFormat = function () {
  return new Date(this.valueOf() + 8 * 3600000)
    .toISOString()
    .replace(/T|\:/g, "-")
    .replace(/(\.(.*)Z)/g, "")
    .split("-")
...
const addOrderRow = async (
  ...
  return new Promise(async (resolve, reject) => {
    wlogger.debug(`\n-----------==addOrderRow`);
    wlogger.debug(
      `inside addOrderRow. paymentStatus: ${paymentStatus}, symbol: ${symbol}`
    );
    ...
    wlogger.debug(
      `orderId: ${orderId}, nationalId: ${nationalId}, nationalIdLast5: ${nationalIdLast5}`
    );
    ...
    const result = await mysqlPoolQueryB(queryStr1, sqlObject).catch((err) =>
      reject("[Error @ mysqlPoolQueryB()]" + err)
    );
    wlogger.debug(`result: ${JSON.stringify(result, null, 4)}`);
    resolve(true);
...
const addUserArrayOrdersIntoDB = async (
  users,
  _price,
  paymentStatus,
  tokenSymbol
  ...
  tokenSymbol: ${tokenSymbol}, fundCount: ${fundCount}, paymentStatus: ${paymentStatus}`);
      ...
      const result = await addOrderRow(
        identityNumber,
        email,
        tokenCount,
        tokenSymbol,
...
const addArrayOrdersIntoDB = async (
  userIndexArray,
  amountArray,
  initialAssetPricing,
  paymentStatus,
...
identityNumber: ${identityNumber}, email: ${email}, tokenCount: ${tokenCount}, 
tokenSymbol: ${tokenSymbol}, fundCount: ${
        initialAssetPricing * tokenCount
...
//-----------------------------==IncomeArrangement
const addActualPaymentTime = async (
  actualPaymentTime,
  symbol,
  payablePeriodEnd
) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr =
      "UPDATE income_arrangement SET ia_actualPaymentTime = ? WHERE ia_SYMBOL = ? AND ia_Payable_Period_End = ?";
    const result = await mysqlPoolQueryB(queryStr, [
      actualPaymentTime,
      symbol,
      payablePeriodEnd,
    ]).catch((err) => {
...
const addIncomeArrangementRowFromObj = (iaObj) => {
  return new Promise(async (resolve, reject) => {
    ...
    const sqlObject = {
      ...
      ia_wholecase_Forecasted_Annual_Income:
        iaObj.wholecaseForecastedAnnualIncome,
      ia_wholecase_Forecasted_Payable_Income_in_the_Period:
        iaObj.wholecaseForecastedPayableIncome,
      ...
      ia_single_Forecasted_Payable_Income_in_the_Period:
        iaObj.singleForecastedPayableIncome,
      ia_single_Actual_Income_Payment_in_the_Period:
        iaObj.singleActualIncomePayment,
      ...
      ia_single_Calibration_Actual_Income_Payment_in_the_Period:
        iaObj.singleCalibrationActualIncome,
    };
    wlogger.debug(sqlObject);
    ...
    wlogger.debug(
      `income arrangement table has been added with one new row. \nresult: ${JSON.stringify(
        result,
        null,
        4
      )}`
...
const addIncomeArrangementRow = (
  ...
  payablePeriodEnd,
  annualEnd,
  wholecasePrincipalCalledBack,
  wholecaseBookValue,
  wholecaseForecastedAnnualIncome,
  wholecaseForecastedPayableIncome,
  wholecaseAccumulatedIncome,
  wholecaseIncomeReceivable,
  wholecaseTheoryValue,
  singlePrincipalCalledBack,
  singleForecastedAnnualIncome,
  singleForecastedPayableIncome,
  singleActualIncomePayment,
  singleAccumulatedIncomePaid,
  singleTokenMarketPrice,
  ia_state,
  singleCalibrationActualIncome
) => {
  return new Promise(async (resolve, reject) => {
    wlogger.debug(
...
const updateIAassetRecordStatus = (symbol) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr =
      "UPDATE income_arrangement SET ia_assetRecord_status = 1 WHERE ia_SYMBOL = ? AND ia_Payable_Period_End = 0";
    const result = await mysqlPoolQueryB(queryStr, [symbol]).catch((err) => {
      wlogger.error(`[Error @ mysqlPoolQueryB(queryStr)]`);
      reject(err);
      return false;
...
const getPastScheduleTimes = async (symbol, serverTime) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "SELECT ia_time FROM income_arrangement WHERE ia_SYMBOL = ? AND ia_time <= ?";
    const result = await mysqlPoolQueryB(queryStr1, [symbol, serverTime]).catch(
      (err) => {
...
const getSymbolsONM = async () => {
  return new Promise(async (resolve, reject) => {
    ...
    let mesg;
    const queryStr2 =
      'SELECT sc_symbol, smart_contracts.sc_erc721address FROM smart_contracts WHERE sc_symbol IN (SELECT p_SYMBOL FROM product WHERE p_state = "ONM")';
    ...
    if (!Array.isArray(result) || !result.length) {
      ...
      for (let i = 0; i < result.length; i++) {
        if (
          ...
          result[i] !== null &&
          !excludedSymbols.includes(result[i].sc_symbol)
        ) {
...
const checkIaAssetRecordStatus = async (symbol, actualPaymentTime) => {
  return new Promise(async (resolve, reject) => {
    ...
    const query1 =
      "SELECT ia_assetRecord_status FROM income_arrangement WHERE ia_SYMBOL = ? AND ia_actualPaymentTime = ?";
    const result = await mysqlPoolQueryB(query1, [
      symbol,
      actualPaymentTime,
    ]).catch((err) => {
    ...
    wlogger.debug(
      `[end of checkIaAssetRecordStatus()] assetRecordStatus: ${assetRecordStatus}`
    );
...
const getMaxActualPaymentTime = async (symbol, serverTime) => {
  return new Promise(async (resolve, reject) => {
    ...
    const query1 =
      "SELECT MAX(ia_actualPaymentTime) FROM income_arrangement WHERE ia_SYMBOL = ? AND ia_assetRecord_status = 0 AND ia_actualPaymentTime <= ?";
    //SELECT MAX(ia_actualPaymentTime) FROM income_arrangement WHERE ia_SYMBOL = "ACHM6666" AND ia_actualPaymentTime <= 201906271235;

    const result = await mysqlPoolQueryB(query1, [symbol, serverTime]).catch(
      (err) => {
        wlogger.error(
    ...
    if (!Array.isArray(result) || !result.length) {
      wlogger.debug(
        `maxActualPaymentTime was not found ... either in the past or in the future...`
      );
      ...
      wlogger.debug(
        `multiple maxActualPaymentTimes are found. \n${resultObjStr}`
      );
    }
    wlogger.debug(
      `[end of getMaxActualPaymentTime()] maxActualPaymentTime: ${maxActualPaymentTime} ${typeof maxActualPaymentTime} \nsymbol: ${symbol}, serverTime: ${serverTime}`
    );
...
const getAcPayment = async (symbol, maxActualPaymentTime) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (maxActualPaymentTime === "0") {
      acPayment = -9;
      wlogger.debug(
        `[end of getAcPayment()] symbol: ${symbol}, maxActualPaymentTime: ${maxActualPaymentTime}`
      );
      resolve(acPayment);
      return false;
    }
    ...
    const query1 =
      'SELECT ia_single_Actual_Income_Payment_in_the_Period FROM income_arrangement WHERE ia_State = "ia_state_approved" AND ia_assetRecord_status = 0 AND ia_SYMBOL = ? AND ia_actualPaymentTime = ?';
    ...
    const result1 = await mysqlPoolQueryB(query1, [
      symbol,
      maxActualPaymentTime,
    ]).catch((err) => {
      wlogger.error(`\n[Error @ getAcPayment()]`);
      reject(err);
      return false;
    ...
    if (!Array.isArray(result1)) {
      ...
      const query2 =
        "SELECT * FROM income_arrangement WHERE ia_SYMBOL = ? AND ia_actualPaymentTime = ?";

      const result2 = await mysqlPoolQueryB(query2, [
        symbol,
        maxActualPaymentTime,
      ]).catch((err) => {
      ...
      if (!Array.isArray(result2) || !result2.length) {
        ...
        if (result2[0]["ia_assetRecord_status"] !== 0) {
          wlogger.warn(
            `>>> ia_assetRecord_status is not 0 => such assetRecord has already be recorded`
          );
        }
        acPayment = -1;
      } else {
      ...
      const result1b =
        result1[0]["ia_single_Actual_Income_Payment_in_the_Period"];
      if (result1b === null) {
        acPayment = -2;
      } else {
        acPayment = result1b;
...
const getProfitSymbolAddresses = async (serverTime) => {
  return new Promise(async (resolve, reject) => {
    wlogger.debug(
      `-----------==getProfitSymbolAddress \nserverTime: ${serverTime} typeof: ${typeof serverTime}`
    );
    const [foundSymbols, foundHCAT721Addrs] = await getSymbolsONM().catch(
      (err) => {
    ...
    await asyncForEach(foundSymbols, async (symbol, index) => {
      const maxActualPaymentTime = await getMaxActualPaymentTime(
        symbol,
        serverTime
      );
      ...
      if (maxActualPaymentTime === "0") {
        ...
        if (acPayment < 0 || acPayment === null || acPayment === undefined) {
          ...
          wlogger.warn(
            `[Warning] ${mesg}: ${acPayment} ${typeof acPayment}, symbol: ${symbol}`
          );
        } else {
...
const calculateLastPeriodProfit = async (serverTime) => {
  return new Promise(async (resolve, reject) => {
    ...
    const [
      ...
      maxActualPaymentTimeArray,
      mesgArray,
    ] = await getProfitSymbolAddresses(serverTime).catch((err) => {
    ...
    if (symbolsLength !== checkedTokenAddrs.length) {
      reject(
        "[Error] checkedSymbols and checkedTokenAddrs are of difference length"
      );
      return false;
    } else if (acPaymentArray.length !== checkedSymbols.length) {
      wlogger.debug(`${acPaymentArray.length},  ${checkedSymbols.length}`);
      reject(
        "[Error] acPaymentArray and checkedSymbols are of different length"
      );
      return false;
      ...
      await asyncForEach(checkedSymbols, async (symbol, index) => {
        if (mesgArray[index]) {
          wlogger.warn(
            `[Warning] at symbol: ${symbol}, mesg: ${mesgArray[index]}`
          );
          ...
          const [toAddressArray, amountArray] = await getOwnerAddrAmountList(
            checkedTokenAddrs[index],
            0,
            0
          );
          ...
          const [emailArrayError, amountArrayError] =
            await addAssetRecordRowArray(
              toAddressArray,
              amountArray,
              symbol,
              ar_time,
          ...
          const resultsetAssetRecordStatus = await setAssetRecordStatus(
            1,
            symbol,
            ar_time
          );
 ...
 */
const getOwnerAddrAmountList = async (
  tokenAddress,
  indexStart,
  indexAmount
) => {
  return new Promise(async (resolve, reject) => {
    ...
    const toAddressArray = await instHCAT721.methods
      .getOwnersByOwnerIndex(indexStart, indexAmount)
      .call();
    const amountArray = await instHCAT721.methods
      .balanceOfArray(toAddressArray)
      .call();
    wlogger.debug(
...
const setAssetRecordStatus = async (
  assetRecordStatus,
  symbol,
  actualPaymentTime
) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "UPDATE income_arrangement SET ia_assetRecord_status = ? WHERE ia_SYMBOL = ? AND ia_actualPaymentTime = ?";
    const result = await mysqlPoolQueryB(queryStr1, [
      assetRecordStatus,
      symbol,
      actualPaymentTime,
...
const addAssetRecordRow = async (
  investorEmail,
  symbol,
  ar_time,
  holdingAmount,
  AccumulatedIncomePaid,
  UserAssetValuation,
  HoldingAmountChanged,
  HoldingCostChanged,
  AcquiredCost,
  MovingAverageofHoldingCost
) => {
  return new Promise(async (resolve, reject) => {
    wlogger.debug(`\n-------==addAssetRecordRow`);
    const sql = {
...
const deleteAssetRecordRows = (tokenSymbol) => {
  return new Promise(async (resolve, reject) => {
    const queryStr1 =
      "DELETE FROM investor_assetRecord WHERE ar_tokenSYMBOL = ?";
    const result = await mysqlPoolQueryB(queryStr1, [tokenSymbol]).catch(
      (err) => {
        reject("[Error @ mysqlPoolQueryB] " + err);
      }
...
const deleteAllRecordsBySymbol = async (tokenSymbol) => {
  return new Promise(async (resolve, reject) => {
    ...
    const result5 = await deleteIncomeArrangementRows(tokenSymbol).catch(
      (err) => {
        reject(err);
      }
    );
    ...
    wlogger.debug(
      `check1 ... result1: ${result1}, result2: ${result2}, result3: ${result3}, result4: ${result4}, result5: ${result5}, result6: ${result6}`
    );

    const result_getProductRows = await getProductRows(tokenSymbol).catch(
      (err) => {
        reject(err);
      }
    );
    ...
    const result_getSmartContractRows = await getSmartContractRows(
      tokenSymbol
    ).catch((err) => {
      reject(err);
    });
...
const addAssetRecordRowArray = async (
  inputArray,
  amountArray,
  symbol,
  ar_time,
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (inputArray[0].includes("@")) {
      ...
      const queryStr4 =
        "SELECT u_email FROM user WHERE u_assetbookContractAddress = ?";
      await asyncForEachAssetRecordRowArray(
        inputArray,
        async (addrAssetbook, index) => {
          const emailObj = await mysqlPoolQueryB(queryStr4, [
            addrAssetbook,
          ]).catch((err) => {
    ...
    await asyncForEachAssetRecordRowArray2(emailArray, async (email, idx) => {
      ...
      if (!email.includes("@")) {
        ...
        wlogger.error(
          `[Error @ email] email: ${email}, amount: ${amount} ... added to emailArrayError and amountArrayError`
        );
      } else {
        wlogger.debug(
          `mysql612: email: ${email}, symbol: ${symbol}, ar_time: ${ar_time} \nacPayment(singleActualIncomePayment): ${acPayment}, amount: ${amount}, acquiredCost: ${acquiredCost}`
        );
        const sqlObject = {
        ...
        const result6 = await mysqlPoolQueryB(queryStr6, sqlObject).catch(
          (err) => {
...
const getFundingStateDB = (symbol) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr2 =
      "SELECT p_state, p_CFSD, p_CFED FROM product WHERE p_SYMBOL = ?";
    ...
    if (!Array.isArray(result) || result.length < 3) {
      ...
      wlogger.debug(
        `symbol: ${symbol}, pstate: ${result[0]}, CFSD: ${result[1]}, CFED: ${result[2]}`
      );
      resolve(true);
    }
  });
...
const setFundingStateDB = (symbol, pstate, CFSD, CFED) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "UPDATE product SET p_state = ?, p_CFSD = ?, p_CFED = ? WHERE p_SYMBOL = ?";
    ...
    if (isGoodCFSD && isGoodCFED) {
      const result1 = await mysqlPoolQueryB(queryStr1, [
        pstate,
        CFSD,
        CFED,
        symbol,
      ...
      wlogger.debug(
        `[setFundingStateDB] symbol: ${symbol}, pstate: ${pstate}, CFSD: ${CFSD}, CFED: ${CFED}`
      );
      //wlogger.debug(`result1: ${result1)`);
      resolve(true);
    } else {
      const result2 = await mysqlPoolQueryB(queryStr2, [pstate, symbol]).catch(
        (err) => {
          wlogger.error(
            `\n[Error @ setTokenStateDB: mysqlPoolQueryB(queryStr2)]`
          );
      ...
      wlogger.info(
        `[Success @setFundingStateDB] have set symbol: ${symbol}, to p_state = ${pstate}`
      );
      //wlogger.debug(`result2: ${result2}`);
      resolve(true);
    }
...
const setTokenStateDB = (symbol, tokenState, lockuptime, validdate) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "UPDATE product SET p_tokenState = ?, p_lockuptime = ?, p_validdate = ? WHERE p_SYMBOL = ?";
    const queryStr2 = "UPDATE product SET p_tokenState = ? WHERE p_SYMBOL = ?";

    const [isGoodlockuptime, lockuptime_, mesglockuptime] =
      testInputTime(lockuptime);
    wlogger.debug("isGoodlockuptime:", isGoodlockuptime);
    const [isGoodvaliddate, validdate_, mesgvaliddate] =
      testInputTime(validdate);
    ...
    if (isGoodlockuptime && isGoodvaliddate) {
      ...
      wlogger.debug(
        `[DB] symbol: ${symbol}, tokenState: ${tokenState}, lockuptime: ${lockuptime}, validdate: ${validdate}`
      );
      //wlogger.debug(`result: ${result1}`);
      resolve(true);
    } else {
      const result = await mysqlPoolQueryB(queryStr2, [
        tokenState,
        symbol,
      ]).catch((err) => {
        wlogger.error(
...
const getTokenStateDB = async (symbol) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr2 =
      "SELECT p_tokenState, p_lockuptime, p_validdate FROM product WHERE p_SYMBOL = ?";
    const result = await mysqlPoolQueryB(queryStr2, [symbol]).catch((err) => {
      wlogger.error(`\n[Error @ setTokenStateDB: mysqlPoolQueryB(queryStr2)]`);
      reject(err);
      return false;
    ...
    resolve([
      true,
      result[0].p_tokenState,
      result[0].p_lockuptime,
      result[0].p_validdate,
    ]);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!Array.isArray(ctrtAddrResult)) {
      ...
      resolve([
        false,
        undefined,
        `[Error] no contract row is found for ${symbol}`,
      ]);
    } else if (ctrtAddrResult.length > 1) {
      resolve([
        false,
        "multipleAddr",
        `[Error] multiple contract rows are found for ${symbol}`,
      ]);
    } else {
      ...
      resolve([
        ctrtAddrObj["sc_crowdsaleaddress"],
        ctrtAddrObj["sc_erc721Controller"],
        ctrtAddrObj["sc_erc721address"],
        ctrtAddrObj["sc_incomeManagementaddress"],
      ]);
    }
  });
};
...
const getCtrtAddr = async (symbol, ctrtType) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "SELECT " + scColumnName + " FROM smart_contracts WHERE sc_symbol = ?";
    const ctrtAddrResult = await mysqlPoolQueryB(queryStr1, [symbol]).catch(
      (err) => {
    ...
    if (!Array.isArray(ctrtAddrResult)) {
      resolve([
        false,
        undefined,
        `[Error] invalid ${ctrtType} contract address found for ${symbol}`,
      ]);
    } else if (ctrtAddrResult.length == 0) {
      resolve([
        false,
        undefined,
        `[Error] no ${ctrtType} contract address is found for ${symbol}`,
      ]);
    } else if (ctrtAddrResult.length > 1) {
      resolve([
        false,
        "multipleAddr",
        `[Error] multiple ${ctrtType} addresses are found for ${symbol}`,
      ]);
    } else {
      ...
      if (isEmpty(targetAddr)) {
        resolve([
          false,
          "0x0",
          `[Error] empty ${ctrtType} contract address value is found for ${symbol}, targetAddr: ${targetAddr}`,
        ]);
      } else {
        resolve([
          true,
          targetAddr,
          `[Good] one ${ctrtType} address is found for ${symbol}`,
        ]);
      }
    }
  });
...
const getSymbolFromCtrtAddr = async (ctrtAddr, ctrtType) => {
  return new Promise(async (resolve, reject) => {
    ...
    const queryStr1 =
      "SELECT sc_symbol from smart_contracts where " + scColumnName + " = ?";
    const symbolResult = await mysqlPoolQueryB(queryStr1, [ctrtAddr]).catch(
      (err) => {
    ...
    if (!Array.isArray(result)) {
      resolve([
        false,
        undefined,
        `[Error] invalid symbol is found for ${ctrtType} contract address$ {ctrtAddr}`,
      ]);
    } else if (symbolResult.length == 0) {
      resolve([
        false,
        undefined,
        `[Error] no symbol is found for ${ctrtType} contract address$ {ctrtAddr}`,
      ]);
    } else if (symbolResult.length > 1) {
      resolve([
        false,
        "multipleSymbols",
        `[Error] multiple symbols are found for ${ctrtType} addresses ${ctrtAddr}`,
      ]);
    } else {
      ...
      if (isEmpty(symbol)) {
        resolve([
          false,
          "",
          `[Error] empty symbol value is found for ${ctrtType} contract address ${ctrtAddr}`,
        ]);
      } else {
        resolve([
          true,
          symbol,
          `[Good] one symbol is found for ${ctrtType} address ${ctrtAddr}`,
        ]);
      }
    }
  });
...
function setIMScheduleDB(symbol, tokenState, lockuptime, validdate) {
  return new Promise(async (resolve, reject) => {
    ...
    if (isGoodlockuptime && isGoodvaliddate) {
      ...
      wlogger.debug(
        `[DB] symbol: ${symbol}, tokenState: ${tokenState}, lockuptime: ${lockuptime}, validdate: ${validdate}, result: ${result}`
      );
      resolve(true);
    } else {
      const queryStr1 =
        "UPDATE product SET p_tokenState = ? WHERE p_SYMBOL = ?";
      const result = await mysqlPoolQueryB(queryStr1, [
        tokenState,
        symbol,
      ]).catch((err) => {
      ...
      wlogger.debug(
        `[DB] symbol: ${symbol}, tokenState: ${tokenState}, result: ${result}`
      );
      resolve(true);
    }
  });
...
function isIMScheduleGoodDB(symbol) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!Array.isArray(result) || result.length < 3) {
      wlogger.debug(
        `[isIMScheduleGoodDB] returned invalid result. symbol: ${symbol}`
      );
      resolve([undefined, undefined]);
    } else {
      wlogger.debug(
        `symbol: ${symbol}, tokenState: ${result[0]}, lockuptime: ${result[1]}, validdate: ${result[2]}`
      );
...
const addIncomePaymentPerPeriodIntoDB = async (serverTime) => {
  wlogger.debug(
    `inside addIncomePaymentPerPeriodIntoDB()... serverTime: ${serverTime} ${typeof serverTime}`
  );
  const symbolArray = [];
  ...
  const queryStr7 =
    "SELECT ia_SYMBOL, ia_actualPaymentTime, ia_single_Actual_Income_Payment_in_the_Period FROM income_arrangement WHERE ia_actualPaymentTime = (SELECT  MAX(ia_actualPaymentTime) FROM income_arrangement WHERE ia_SYMBOL = ?)";
  await asyncForEach(symbolObjArray, async (symbolObj, index) => {
    const result = await mysqlPoolQueryB(queryStr7, [
      symbolObj.ia_SYMBOL,
    ]).catch((err) => {
    ...
    if (!Array.isArray(result) || !result.length) {
      ...
      if (serverTime >= acpaymentTime) {
        ...
        const incomePayment = parseInt(
          result[0].ia_single_Actual_Income_Payment_in_the_Period
        );
  ...
  await asyncForEach(symbolArray, async (symbol, index) => {
    ...
    if (!Array.isArray(result2)) {
      ...
      wlogger.error(
        `[Error] erc21 contract addresses is null or undefined or empty string`
      );
      addrHCAT_Array.push(result2[0].sc_erc721address);
    } else {
      wlogger.info(
        `[Good] erc721 contract address: ${result2[0].sc_erc721address}`
      );
      addrHCAT_Array.push(result2[0].sc_erc721address);
    }
  });
  ...
  await asyncForEach(addrHCAT_Array, async (tokenCtrtAddr, index) => {
    if (
      tokenCtrtAddr !== null ||
      tokenCtrtAddr !== undefined ||
      tokenCtrtAddr !== "multiple contract addr" ||
      tokenCtrtAddr !== "Not on record"
    ) {
      const [abAddrArray, abBalArray] = await getOwnerAddrAmountList(
        tokenCtrtAddr,
        0,
        0
      );
  ...
  await asyncForEach(abAddrArrayGroup, async (abAddrArray, index) => {
    ...
    await asyncForEach(abAddrArray, async (assetbookAddr, idx) => {
      const queryStr3 =
        "SELECT u_email FROM user WHERE u_assetbookContractAddress = ?";
      const result3 = await mysqlPoolQueryB(queryStr3, [assetbookAddr]).catch(
        (err) => {
...
const getForecastedSchedulesFromDB = async (symbol) => {
  return new Promise(async (resolve, reject) => {
    const queryStr1 =
      "SELECT ia_time, ia_single_Forecasted_Payable_Income_in_the_Period From income_arrangement where ia_SYMBOL = ?";
    const result = await mysqlPoolQueryB(queryStr1, [symbol]).catch((err) => {
    ...
    if (!Array.isArray(result)) {
      reject(
        "invalid result from ia_time, ia_single_Forecasted_Payable_Income_in_the_Period"
      );
      return false;
    } else if (result.length === 0) {
      reject(
        "ia_time, ia_single_Forecasted_Payable_Income_in_the_Period not found"
      );
      return false;
    } else {
      ...
      for (let i = 0; i < result.length; i++) {
        if (
          ...
          forecastedPayableAmounts.push(
            result[i].ia_single_Forecasted_Payable_Income_in_the_Period
          );
        } else {
          forecastedPayableTimesError.push(result[i].ia_time);
          forecastedPayableAmountsError.push(
            result[i].ia_single_Forecasted_Payable_Income_in_the_Period
          );
        }
      }
