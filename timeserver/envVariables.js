...
if (blockchainChoice === "1") {
...
} else if (blockchainChoice === "2") {
  /*ganache*/
  blockchainURL =
    "http://" + process.env.BC_HOST + ":" + process.env.BC_PORT_GANACHE;
  gasLimitValue = 9000000; // for POW private chain
  gasPriceValue = 20000000000; //100000000000000000
  addrHelium = "";
  addrRegistry = "";
  addrProductManager = "";
} else if (blockchainChoice === "3") {
  ...
  addrRegistry = "";
  addrProductManager = "";
} else if (blockchainChoice === "4") {
  /*local ganache-cli
  ganache-cli -m "mnemonic words..." -l 9721975  -g 20000000000 */
  blockchainURL =
    "http://" +
    process.env.BC_HOST_GANACHECLI +
    ":" +
    process.env.BC_PORT_GANACHECLI;
...
const is_updateFundingStateFromDB =
  process.env.IS_UPDATE_FUNDING_STATE_FROM_DB === "1";
//crowdfunding state can be partially triggered by timeserver!!!

const is_updateTokenStateFromDB =
  process.env.IS_UPDATE_TOKEN_STATE_FROM_DB === "1";
const is_calculateLastPeriodProfit =
  process.env.IS_CALCULATE_LAST_PERIOD_PROFIT === "1";
...
module.exports = {
  ...
  is_addAssetbooksIntoCFC,
  is_makeOrdersExpiredCFED,
  is_updateExpiredOrders,
  is_updateFundingStateFromDB,
  is_updateTokenStateFromDB,
  is_calculateLastPeriodProfit,
  loglevel,
};
