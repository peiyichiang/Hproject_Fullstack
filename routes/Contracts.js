...
const management = [
  process.env.HELIUM_ADMIN,
  process.env.HELIUM_CHAIRMAN,
  process.env.HELIUM_DIRECTOR,
  process.env.HELIUM_MANAGER,
  process.env.HELIUM_OWNER,
];
...
router.post(
  "/heliumContract/isAfterDeployment",
...
router.post(
  "/heliumContract/checkCustomerService",
  async function (req, res, next) {
    const addrHelium = req.body.addrHelium;
    const addrInput = req.body.addrInput;
    console.log(
      `inside checkCustomerService... \naddrHelium: ${addrHelium}\naddrInput: ${addrInput}`
    );
    const instHelium = new web3.eth.Contract(heliumContract.abi, addrHelium);
    const result = await instHelium.methods
      .checkCustomerService(addrInput)
      .call();
    res.send({ result });
  }
);
...
router.post(
  "/heliumContract/checkPlatformSupervisor",
  async function (req, res, next) {
    const addrHelium = req.body.addrHelium;
    const addrInput = req.body.addrInput;
    console.log(
      `inside checkPlatformSupervisor... \naddrHelium: ${addrHelium}\naddrInput: ${addrInput}`
    );
    const instHelium = new web3.eth.Contract(heliumContract.abi, addrHelium);
    const result = await instHelium.methods
      .checkPlatformSupervisor(addrInput)
      .call();
    res.send({ result });
  }
);
...
router.post("/registryContract/users/:u_id", async function (req, res, next) {
  ...
  let encodedData = registry.methods
    .addUser(userID, assetBookAddr, 1)
    .encodeABI();
  console.log("User assetbook addr is wrting back to the DB2");
  try {
    let contractResult = await signTx(
      backendAddr,
      backendRawPrivateKey,
      registryContractAddr,
      encodedData
    );
    /**寫入DataBase */
...
router.get("/registryContract/users/:u_id", async function (req, res, next) {
  ...
  let userInfo = await registry.methods
    .getUser(u_id)
...
router.post(
  "/registryContract/getUserFromUid/:uid",
  async function (req, res, next) {
...
router.get(
  "/registryContract/getUserFromAssetbook/:assetbook",
  async function (req, res, next) {
    ...
    const isContract = await instRegistry.methods
      .getUserFromAssetbook(assetbook)
      .call();
    res.send({ isContract });
  }
);
...
router.post(
  "/registryContract/setRestrictions",
  async function (req, res, next) {
    ...
    console.log(
      `${authLevel}: ${authLevel}, maxBuyAmountPublic: ${maxBuyAmountPublic}, maxBalancePublic: ${maxBalancePublic}, maxBuyAmountPrivate: ${maxBuyAmountPrivate}, maxBalancePrivate: ${maxBalancePrivate}`
    );
    ...
    const restrictionsResult = await setRestrictions(
      registryContractAddr,
      authLevel,
      maxBuyAmountPublic,
      maxBalancePublic,
...
router.get(
  ...
  async function (req, res, next) {
    ...
    const antiPlatformOverrideDaysDefault = await instAssetbook.methods
      .antiPlatformOverrideDaysDefault()
      .call();
    const antiPlatformOverrideDays = await instAssetbook.methods
    ...
    const checkAssetOwner = await instAssetbook.methods
      .checkAssetOwner()
      .call();
    const checkCustomerService = await instAssetbook.methods
      .checkCustomerService()
      .call();
    const isAblePlatformOverride = await instAssetbook.methods
...
router.post("/assetbook/endorsers", async function (req, res, next) {
  ...
  const endorserAddr = await instAssetbook.methods
...
router.post("/assetbook/checkIsContract", async function (req, res, next) {
  ...
  const isContract = await instAssetbook.methods
    .checkIsContract(targetAddress)
    .call();
...
router.post("/assetbook/getAsset", async function (req, res, next) {
  ...
  const assetInfo = await instAssetbook.methods
    .getAsset(assetIndex, tokenAddress)
    .call();
...
router.post(
  "/crowdFundingContract/:tokenSymbol",
  async function (req, res, next) {
    const provider = new PrivateKeyProvider(backendPrivateKey, blockchainURL);
    const web3deploy = new Web3(provider);
    ...
    crowdFunding
      ...
      .on("receipt", function (receipt) {
        ...
        mysqlPoolQuery(
          "INSERT INTO smart_contracts (sc_symbol, sc_crowdsaleaddress, sc_totalsupply, sc_remaining) VALUES (?,?,?,?)",
          [tokenSymbol, contractAddress, quantityMax, quantityMax],
          function (err, rows) {
            if (err) {
              console.log(err);
              res.send({
...
/**invest*/
router.post(
  "/crowdFundingContract/:tokenSymbol/investors/:assetBookAddr",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    let currentTime;
    ...
    mysqlPoolQuery(
      "SELECT sc_crowdsaleaddress FROM smart_contracts WHERE sc_symbol = ?",
      [tokenSymbol],
      async function (err, DBresult, rows) {
        if (err) {
          //console.log(err);
          res.send({
            err: err,
          ...
          let crowdFunding = new web3.eth.Contract(
          ...
          /*用後台公私鑰sign*/
          let encodedData = crowdFunding.methods
            .invest(assetBookAddr, quantityToInvest, currentTime)
            .encodeABI();
          try {
            ...
            console.log("error:" + error);
            let revertReason = await crowdFunding.methods
              .checkInvestFunction(assetBookAddr, quantityToInvest, currentTime)
              .call({ from: backendAddr });
...
/**更新剩餘數量 in DB */
router.post(
  "/crowdFundingContract/:tokenSymbol/remaining",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    ...
    mysqlPoolQuery(
      "SELECT sc_crowdsaleaddress FROM smart_contracts WHERE sc_symbol = ?",
      [tokenSymbol],
      async function (err, DBGetResult, rows) {
        if (err) {
          //console.log(err);
          res.send({
            err: err,
          ...
          /*用後台公私鑰sign*/
          let quantitySold = await crowdFunding.methods
            .quantitySold()
            .call({ from: backendAddr });
          let maxTotalSupply = await crowdFunding.methods
            .maxTotalSupply()
            .call({ from: backendAddr });
          ...
          mysqlPoolQuery(
            "UPDATE `htoken`.`smart_contracts` SET `sc_remaining` = ? WHERE (`sc_symbol` = ?)",
            [remaining, tokenSymbol],
            async function (err, DBUpdateResult, rows) {
              if (err) {
                res.send({
...
/**get investors */
router.get(
  "/crowdFundingContract/:tokenSymbol/investors",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    ...
    mysqlPoolQuery(
      ...
      async function (err, DBresult, rows) {
        if (err) {
          ...
          let investors = await crowdFunding.methods
            .getInvestors(1, 100)
            .call({ from: backendAddr });
...
/**funding pause*/
router.post(
  "/crowdFundingContract/:tokenSymbol/pause",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    let currentTime;
...
/**funding resume*/
router.post(
  "/crowdFundingContract/:tokenSymbol/resume",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    let currentTime;
    ...
    mysqlPoolQuery(
      ...
      async function (err, DBresult, rows) {
        if (err) {
          ...
          /*用後台公私鑰sign*/
          let encodedData = crowdFunding.methods
            .resumeFunding(CFED2, quantityMax, currentTime)
            .encodeABI();
          ...
          try {
            ...
            console.log("error:" + error);
            let revertReason = await crowdFunding.methods
              .checkResumeFunding(CFED2, quantityMax, currentTime)
              .call({ from: backendAddr });
            console.log("revertReason:" + revertReason);
            res.status(500);
            res.send({ error: error.toString(), revertReason: revertReason });
...
/**funding terminate*/
router.post(
  "/crowdFundingContract/:tokenSymbol/terminate",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    let currentTime;
...
/**funding changeCFED*/
router.post(
  "/crowdFundingContract/:tokenSymbol/changeCFED",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    let currentTime;
...
/**get status */
router.get(
  "/crowdFundingContract/:tokenSymbol/status",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    ...
    mysqlPoolQuery(
      ...
      async function (err, DBresult, rows) {
        if (err) {
          ...
          let fundingState = await crowdFunding.methods
            .fundingState()
            .call({ from: backendAddr });
          let stateDescription = await crowdFunding.methods
            .stateDescription()
            .call({ from: backendAddr });
          let quantityGoal = await crowdFunding.methods
            .quantityGoal()
            .call({ from: backendAddr });
          let maxTotalSupply = await crowdFunding.methods
            .maxTotalSupply()
          ...
          let quantitySold = await crowdFunding.methods
            .quantitySold()
            .call({ from: backendAddr });
          let CFSD2 = await crowdFunding.methods
            .CFSD()
            .call({ from: backendAddr });
          let CFED2 = await crowdFunding.methods
            .CFED()
            .call({ from: backendAddr });
...
router.post(
  "/crowdFundingContract/:tokenSymbol/updateState",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
...
/**close funding*/
router.post(
  "/crowdFundingContract/:tokenSymbol/closeFunding",
  async function (req, res, next) {
    const provider = new PrivateKeyProvider(backendPrivateKey, blockchainURL);
    const web3deploy = new Web3(provider);
    ...
    const tokenController = new web3deploy.eth.Contract(
      tokenControllerContract.abi
    );
    const HCAT721 = new web3deploy.eth.Contract(HCAT721_AssetTokenContract.abi);
    const incomeManager = new web3deploy.eth.Contract(
      incomeManagerContract.abi
    );
    const productManager = new web3.eth.Contract(
      productManagerContract.abi,
      productManagerContractAddr
    );
    ...
    await tokenController
      ...
      .on("error", function (error) {
        ...
        res.status(500);
        res.send({
          Title: "tokenController deploy failed",
    ...
    await incomeManager
      ...
      .on("error", function (error) {
        ...
        res.status(500);
        res.send({
          Title: "incomeManager deploy failed",
          Reason: error.toString(),
        });
      });

    let encodedData = productManager.methods
      .addNewCtrtGroup(
        nftSymbolBytes32,
        crowdFundingCtrtAddr,
        tokenControllerAddr,
        HCAT721Addr,
        incomeManagerAddr
      )
      .encodeABI();
    try {
      let combineContractResult = await signTx(
        backendAddr,
        backendRawPrivateKey,
        productManagerContractAddr,
        encodedData
      ...
      console.log(nftSymbol);
      mysqlPoolQuery(
        "UPDATE smart_contracts SET ? WHERE sc_symbol = ?",
        [updateContractsAddrsql, nftSymbol],
        function (err, rows) {
          if (err) {
            console.log(err);
            res.send({
            ...
            mysqlPoolQuery(
              ...
              [updateCrowdFubdingStatesql, nftSymbol],
              function (err, rows) {
                if (err) {
                  console.log(err);
                  res.send(err);
...
router.post("/getTokenSymbolFromCtrtAddr", async function (req, res, next) {
  ...
  const [isGood, symbol, resultMesg] = await findSymbolFromCtrtAddr(
    ctrtAddr,
    ctrtType
  );
...
router.post("/getCtrtAddrFromTokenSymbol", async function (req, res, next) {
  ...
  const [isGood, ctrtAddr, resultMesg] = await getCtrtAddr(
    tokenSymbol,
    ctrtType
  ).catch((err) => {
...
router.post("/getAssetbookFromEmail", async function (req, res, next) {
  ...
  const [isGoodAssetbook, assetbookX, resultMesg] = await getAssetbookFromEmail(
    email
  ).catch((err) => {
  ...
  console.log(
    `----------==API isGoodAssetbook: ${isGoodAssetbook}, assetbookX: ${assetbookX}, resultMesg: ${resultMesg}`
  );
  res.send({
    isGoodAssetbook,
    assetbookX,
...
router.post("/getAssetbookFromIdentityNumber", async function (req, res, next) {
  ...
  const [isGoodAssetbook, assetbookX, resultMesg] =
    await getAssetbookFromIdentityNumber(identityNumber).catch((err) => {
...
router.get(
  "/crowdfunding/getCrowdfundingDetails/:ctrtAddr",
  async function (req, res, next) {
...
router.post("/crowdfunding/getInvestors", async function (req, res, next) {
  ...
  if (
    parseInt(indexStart) < 0 ||
    parseInt(amount) < 0 ||
    isEmpty(crowdfundingCtrtAddr)
  ) {
    res.send({
  ...
  const investors = await instCrowdfunding.methods
    .getInvestors(indexStart, amount)
    .call();
...
router.post("/crowdfunding/emailToQty", async function (req, res, next) {
  ...
  if (isGoodAssetbook) {
    ...
    const quantityOwned = await instCrowdfunding.methods
      .ownerToQty(assetbookX)
      .call();
...
router.post(
  "/tokenControllerContract/:tokenSymbol/updateState",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    let currentTime = req.body.time;
    ...
    mysqlPoolQuery(
      "SELECT sc_erc721Controller FROM smart_contracts WHERE sc_symbol = ?",
      [tokenSymbol],
      async function (err, DBresult, rows) {
        if (err) {
          //console.log(err);
          ...
          let tokenController = new web3.eth.Contract(
            tokenControllerContract.abi,
            tokenControllerAddr
          );

          /*用後台公私鑰sign*/
          let encodedData = tokenController.methods
            .updateState(currentTime)
            .encodeABI();
...
router.get(
  "/tokenControllerContract/:tokenSymbol/status",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
    ...
    mysqlPoolQuery(
      ...
      async function (err, DBresult, rows) {
        if (err) {
          ...
          let status = await tokenController.methods
            .tokenState()
            .call({ from: backendAddr });
...
router.get(
  "/tokenController/getHTokenControllerDetails/:ctrtAddr",
  async function (req, res, next) {
    ...
    const getHTokenControllerDetails = await instTokenController.methods
      .getHTokenControllerDetails()
      .call();
    console.log(`getHTokenControllerDetails: ${getHTokenControllerDetails}`);

    const isTokenApprovedOperational = await instTokenController.methods
...
router.get(
  "/tokenHCAT/getTokenContractDetails/:ctrtAddr",
  async function (req, res, next) {
    ...
    const instHCAT721 = new web3.eth.Contract(
      HCAT721_AssetTokenContract.abi,
      ctrtAddr
    );
    const getTokenContractDetails = await instHCAT721.methods
...
router.post(
  "/tokenHCAT/getOwnersByOwnerIndex",
  async function (req, res, next) {
    const ctrtAddr = req.body.ctrtAddr;
    ...
    const getOwnersByOwnerIndex = await instHCAT721.methods
      .getOwnersByOwnerIndex(startIndex, amount)
      .call();
...
router.post("/tokenHCAT/getTokenIdByIndex", async function (req, res, next) {
  ...
  const getTokenIdByIndex = await instHCAT721.methods
    .getTokenIdByIndex(assetbookAddr, tokenIdAccountIndex)
    .call();
...
router.post("/tokenHCAT/getAccountIds", async function (req, res, next) {
  ...
  const getAccountIds = await instHCAT721.methods
    .getAccountIds(assetbookAddr, startIndex, amount)
    .call();
...
router.post("/tokenHCAT/allowance", async function (req, res, next) {
  ...
  const allowance = await instHCAT721.methods
    .allowance(assetbookAddr, operatorAddr)
    .call();
...
router.post("/tokenHCAT/checkTokenApprove", async function (req, res, next) {
  ...
  const checkTokenApprove = await instHCAT721.methods
    .checkTokenApprove(operatorAddr)
    .call();
...
router.post("/tokenHCAT/tokenApprove", async function (req, res, next) {
  ...
  const tokenApprove = await instHCAT721.methods
    .tokenApprove(operatorAddr, amount)
    .call();
...
router.post("/tokenHCAT/ckStringLength", async function (req, res, next) {
  ...
  const ckStringLength = await instHCAT721.methods
    .ckStringLength(str, minStrLen, maxStrLen)
    .call();
...
router.post(
  "/HCAT721_AssetTokenContract/:nftSymbol",
  async function (req, res, next) {
    const provider = new PrivateKeyProvider(backendPrivateKey, blockchainURL);
    const web3deploy = new Web3(provider);
    ...
    const ERC721SPLC = new web3deploy.eth.Contract(
      HCAT721_AssetTokenContract.abi
    );
...
router.post("/doAssetRecordsCaller/:symbol/", async function (req, res, next) {
  ...
  let isSuccess = false,
    mesg = ""; //PO: 1, PP: 2
  const [
    is_addAssetRecordRowArray,
    emailArrayError,
    amountArrayError,
    is_addActualPaymentTime,
    is_setFundingStateDB,
  ] = await doAssetRecordsCaller(toAddressArray, amountArray, symbol, pricing);
...
router.post(
  "/HCAT721_AssetTokenContract/:symbol/preMint",
  async function (req, res, next) {
    console.log(`\n---------------------==API preMint...`);
    const symbol = req.params.symbol;
    let mesg = "";
  ...
  \naddressArray: ${addressArray} \namountArray: ${amountArray} \ntokenCtrtAddr: ${tokenCtrtAddr}, \npricing: ${pricing}, fundingType: ${fundingType}`);
    ...
    res.send({
      is_preMint: is_preMint,
      mesg_preMint: mesg_preMint,
      addressArray: addressArray,
      amountArray: amountArray,
      tokenCtrtAddr: tokenCtrtAddr,
...
router.post(
  "/HCAT721_AssetTokenContract/:symbol/mintSequentialPerContract",
  async function (req, res, next) {
    ...
    ] = await mintSequentialPerContract(
      symbol,
      serverTime,
      maxMintAmountPerRun
    );
    console.log(
      `is_preMint: ${is_preMint}, is_doAssetRecords: ${is_doAssetRecords}, is_addActualPaymentTime: ${is_addActualPaymentTime}, is_setFundingStateDB: ${is_setFundingStateDB}, is_sequentialMintSuper: ${is_sequentialMintSuper}`
    );
  }
);
...
/** calculateLastPeriodProfit */
router.post(
  "/HCAT721_AssetRecord/:tokenSymbol",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
...
/**get tokenId  */
router.get(
  "/HCAT721_AssetTokenContract/:tokenSymbol/:assetBookAddr",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let assetBookAddr = req.params.assetBookAddr;
    let mysqlPoolQuery = req.pool;
    ...
    mysqlPoolQuery(
      ...
      async function (err, DBresult, rows) {
        if (err) {
          ...
          let HCAT721_AssetToken = new web3.eth.Contract(
            HCAT721_AssetTokenContract.abi,
            contractAddr
          );
          let tokenId = await HCAT721_AssetToken.methods
            .balanceOf(assetBookAddr)
            .call({ from: backendAddr });
...
/**get tokenId  */
router.get(
  "/HCAT721_AssetTokenContract/:tokenSymbol/tokenId",
  async function (req, res, next) {
    let tokenSymbol = req.params.tokenSymbol;
    let mysqlPoolQuery = req.pool;
...
router.post(
  "/HCAT721_AssetTokenContract/safeTransferFromBatch",
  async function (req, res, next) {
    ...
    const _fromAssetOwnerpkRaw =
      "0x2457188f06f1e788fa6d55a8db7632b11a93bb6efde9023a9dbf59b869054dca";
    ...
    try {
      ...
      console.log("error:" + error);
      const revertReason = await instAssetBookFrom.methods
...
router.post(
  "/incomeManagerContract/:nftSymbol",
  async function (req, res, next) {
    const provider = new PrivateKeyProvider(backendPrivateKey, blockchainURL);
    const web3deploy = new Web3(provider);
...
//---------------------------==
router.get(
  "/incomeManagerContract/:tokenSymbol/schCindex",
  async function (req, res, next) {
...
router.get(
  "/incomeManagerContract/:tokenSymbol/getIncomeSchedule",
  async function (req, res, next) {
    const symbol = req.params.tokenSymbol;
    const schIndex = req.params.schIndex;
...
router.get(
  "/incomeManagerContract/:tokenSymbol/getIncomeScheduleList",
  async function (req, res, next) {
    const symbol = req.params.tokenSymbol;
    const forecastedPayableTime = req.params.forecastedPayableTime;
    const scheduleList = await getIncomeScheduleList(
      symbol,
      forecastedPayableTime
    ).catch((err) => {
...
router.get(
  "/incomeManagerContract/:tokenSymbol/checkAddScheduleBatch",
  async function (req, res, next) {
    const symbol = req.params.tokenSymbol;
    ...
    if (isGood) {
      const [array1, array2] = await checkAddScheduleBatch(
        addrIncomeManager,
...
router.post(
  "/incomeManagerContract/:tokenSymbol/addScheduleBatchFromDB",
  async function (req, res, next) {
    const symbol = req.params.tokenSymbol;
    // const forecastedPayableTimes = req.params.forecastedPayableTimes;
    // const forecastedPayableAmounts = req.params.forecastedPayableAmounts;
    ...
    if (isGood) {
      ...
      if (result) {
        ...
        const results = await checkAddScheduleBatch(
          addrIncomeManager,
          forecastedPayableTimes,
          forecastedPayableAmounts
        ).catch((err) => {
...
//-------------------==
router.get(
  "/incomeManagerContract/:tokenSymbol/removeIncomeSchedule",
  async function (req, res, next) {
    const symbol = req.params.tokenSymbol;
    const schIndex = req.params.schIndex;
...
//-------------------==
router.get(
  "/incomeManagerContract/:tokenSymbol/imApprove",
  async function (req, res, next) {
    const symbol = req.params.tokenSymbol;
    const schIndex = req.params.schIndex;
...
//-------------------==
router.get(
  "/incomeManagerContract/:tokenSymbol/setPaymentReleaseResults",
  async function (req, res, next) {
    const symbol = req.params.tokenSymbol;
    const schIndex = req.params.schIndex;
    ...
    const result = await setPaymentReleaseResults(
      symbol,
      schIndex,
      actualPaymentTime,
      actualPaymentAmount,
...
router.get(
  "/incomeManagerContract/:tokenSymbol/isScheduleGoodForRelease",
  async function (req, res, next) {
    ...
    mysqlPoolQuery(
      "SELECT sc_incomeManagementaddress FROM smart_contracts WHERE sc_symbol = ?",
      [symbol],
      async function (err, DBresult, rows) {
        if (err) {
          //console.log(err);
          ...
          let incomeManager = new web3.eth.Contract(
            incomeManagerContract.abi,
            incomeManagerAddr
          );
          let isScheduleGoodForRelease = await incomeManager.methods
            .isScheduleGoodForRelease(currentTime)
            .call({ from: backendAddr });
...
//-------------== version 2
router.get(
  "/incomeManagerCtrt/getContractDetails/:ctrtAddr",
  async function (req, res, next) {
    const ctrtAddr = req.params.ctrtAddr;
    console.log(`\nctrtAddr: ${ctrtAddr}`);
    if (isEmpty(ctrtAddr)) {
    ...
    const TimeOfDeployment = await instIncomeManager.methods
      .TimeOfDeployment()
      .call();
    const paymentCount = await instIncomeManager.methods.paymentCount().call();
    console.log(
      `schCindex: ${schCindex}, TimeOfDeployment: ${TimeOfDeployment}, paymentCount: ${paymentCount}`
    );
...
router.post("/incomeManagerCtrt/dateToIdx", async function (req, res, next) {
  ...
  const dateToIdx = await instIncomeManager.methods
    .dateToIdx(schDateTime)
    .call();
...
router.post(
  "/incomeManagerCtrt/idxToSchedule",
  async function (req, res, next) {
    const ctrtAddr = req.body.ctrtAddr;
    ...
    const idxToSchedule = await instIncomeManager.methods
      .idxToSchedule(scheduleIndex)
      .call();
...
router.post(
  "/incomeManagerCtrt/getIncomeSchedule",
  async function (req, res, next) {
    const ctrtAddr = req.body.ctrtAddr;
    ...
    const getIncomeSchedule = await instIncomeManager.methods
      .getIncomeSchedule(input)
      .call();
...
router.post(
  "/incomeManagerCtrt/getIncomeScheduleList",
  async function (req, res, next) {
    //const ctrtAddr = req.body.ctrtAddr;
    //const input = req.body.input;
    ...
    const getIncomeScheduleList = await instIncomeManager.methods
      .getIncomeScheduleList(input, amount)
      .call();
    console.log(
      `getIncomeScheduleList: ${JSON.stringify(getIncomeScheduleList)}`
    );
...
router.post("/productManagerContract", function (req, res, next) {
  ...
  const productManager = new web3deploy.eth.Contract(
    productManagerContract.abi
  );
...
router.post(
  "/productManagerContract/:nftSymbol",
  async function (req, res, next) {
    ...
    let productManager = new web3.eth.Contract(
      productManagerContract.abi,
      contractAddr
    );
...
router.get(
  ...
  async function (req, res, next) {
    ...
    let result = await productManager.methods
      .getCtrtGroup(nftSymbolBytes32)
      .call({ from: backendAddr });
...
/*router.post("/TokenLock", async function (req, res, next) {
  ...
  try {
    ...
    mysqlPoolQuery(
      "SELECT u_assetbookContractAddress,u_assetbookContractAddress2,u_eth_add,u_eth_p FROM user WHERE u_email=?",
      [u_email],
      async function (err, rows) {
        if (err) {
          console.log(err);
          res.send({ success: false, message: "DB query error: " + err });
        } else {
          try {
            result = await transferTokens(
              TokenAddr,
              rows[0].u_assetbookContractAddress,
              rows[0].u_assetbookContractAddress2,
              parseInt(quantity),
              parseInt(priceStr),
              rows[0].u_eth_add,
              rows[0].u_eth_p
            ...
            if (result == false) {
              res.send({
                success: false,
                message: "isTokenApprovedOperational must be true",
              });
            }
            res.send({ success: result, message: "Token Locked Success" });
          } catch (err) {
            if (err.message) {
...
router.post("/TokenLock_txParams", async function (req, res, next) {
  var mysqlPoolQuery = req.pool;
  let u_email = req.body.u_email;
  let quantity = req.body.quantity;
  let TokenAddr = req.body.TokenAddr; //在smartcontract table中
  ...
  try {
    await need_newAccount_ro_not(mysqlPoolQuery, u_email);
    mysqlPoolQuery(
      "SELECT u_assetbookContractAddress,u_assetbookContractAddress2,u_eth_add FROM user WHERE u_email=?",
      [u_email],
      async function (err, rows) {
        if (err) {
          console.log(err);
          ...
          try {
            result = await transferTokens_txParams(
              TokenAddr,
              rows[0].u_assetbookContractAddress,
              rows[0].u_assetbookContractAddress2,
              parseInt(quantity),
            ...
            res.send({
              success: true,
              txParams: result,
              message: "Get Token Lock txParams",
            });
          } catch (err) {
            if (err.message) {
              res.send({ success: false, message: err.message });
...
router.post("/TransferToBuyer", async function (req, res, next) {
  ...
  mysqlPoolQuery(
    "SELECT * FROM user WHERE  u_email =? OR u_email=?",
        ...
        if (err) {
          ...
          if (rows[0].u_email == buyer_email) {
            try {
              result = await transferTokens(
                TokenAddr,
                rows[1].u_assetbookContractAddress2,
                rows[0].u_assetbookContractAddress,
                parseInt(quantity),
                parseInt(price),
                rows[1].u_eth_add2,
          ...
          if (rows[0].u_email == seller_email) {
            try {
              result = await transferTokens(
                TokenAddr,
                rows[0].u_assetbookContractAddress2,
                rows[1].u_assetbookContractAddress,
                parseInt(quantity),
                parseInt(price),
                rows[0].u_eth_add2,
                rows[0].u_eth_p2
...
async function need_newAccount_ro_not(mysqlPoolQuery, u_email) {
  return new Promise(function (resolve, reject) {
    mysqlPoolQuery(
      "SELECT u_assetbookContractAddress2,u_identityNumber FROM user WHERE u_email=?;",
      [u_email],
      async function (err, rows) {
        //檢查是否初次使用二手市場，是否需要生產第二組AssetBook address
        if (err) {
          console.log(err);
        } else {
          if (rows.length == 0) {
            ...
            await RegistryForSecondaryMarketAccount(
              rows[0].u_identityNumber + "SecMarket",
              AssetbookAddr
            );

            mysqlPoolQuery(
              "UPDATE user SET u_eth_p2 =?, u_eth_add2 =?,u_assetbookContractAddress2=? WHERE u_email = ?",
              [EOA[1], EOA[0], AssetbookAddr, u_email],
              function (err) {
                if (err) {
                  reject(err);
                }
