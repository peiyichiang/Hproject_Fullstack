...
router.get('/GET/TxRecord', function (req, res, next) {
    ...
    mysqlPoolQuery('SELECT * FROM tx_record', function (err, rows) {
        if (err) {
            console.log(err);
        }
        var data = rows;
...
router.post('/POST/AddTxRecord', function (req, res, next) {
    ...
    var qur = mysqlPoolQuery('INSERT INTO tx_record SET ?', sql, function (err, rows) {
        if (err) {
            console.log(err);
        }
...
router.get('/GET/DeleteTxRecord', function (req, res, next) {
    ...
    var qur = mysqlPoolQuery('DELETE FROM tx_record WHERE tx_hash = ?', tx_hash, function (err, rows) {
        if (err) {
            console.log(err);
        }
        res.redirect('/GET/TxRecord');
...
router.get('/GET/EditTxRecord', function (req, res, next) {
    ...
    mysqlPoolQuery('SELECT * FROM tx_record WHERE tx_hash = ?', tx_hash, function (err, rows) {
        if (err) {
            console.log(err);
        }
...
router.post('/POST/EditTxRecord', function (req, res, next) {
    ...
    var qur = mysqlPoolQuery('UPDATE tx_record SET ? WHERE tx_hash = ?', [sql, tx_hash], function (err, rows) {
        if (err) {
            console.log(err);
        }
