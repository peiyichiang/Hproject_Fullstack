      ...
      if (result.length === 0) {
        ...
        if (result[0].u_verify_status === 0) {
          res
            .status(400)
            .send(
              "此帳號信箱驗證尚未通過，請收取驗證信並點按驗證連結後再試一次"
            );
          ...
          bcrypt
            ...
            .catch((err) =>
              console.error("Error at compare password & pwHash", err.message)
            );
        }
      } else {
        res.status(400);
...
router.post("/ReviewForgetPassword", function (req, res, next) {
  ...
  var qur = mysqlPoolQuery(
    "UPDATE " +
      process.env.DB_NAME +
      ".forget_pw SET ? WHERE fp_investor_email = ? and fp_application_date=?",
    [sql, req.body.email, req.body.application_date],
    function (err, rows) {
      if (err) {
        ...
        var qur = mysqlPoolQuery(
          "SELECT fp_salt,fp_password_hash From " +
            process.env.DB_NAME +
            ".forget_pw WHERE fp_investor_email = ? and fp_application_date=?",
          [req.body.email, req.body.application_date],
          function (err, rows) {
            if (err) {
              ...
              var qur = mysqlPoolQuery(
                "UPDATE " +
                  process.env.DB_NAME +
                  ".user SET ? WHERE u_email = ?",
                [NewPasswordSQL, req.body.email],
                function (err, rows) {
                  if (err) {
                    res
                      .status(400)
                      .json({ message: "更新salt與pwd_hash失敗" + err });
...
router.post("/reviewStatus", function (req, res, next) {
  ...
  var qur = mysqlPoolQuery(
    "UPDATE " + process.env.DB_NAME + ".user SET ? WHERE u_email = ?",
    [sql, req.body.email],
    function (err, rows) {
      if (err) {
...
router.post("/send_email", function (req, res) {
  ...
  let mailOptions = {
    ...
    text:
      `請點以下連結以完成驗證： ${process.env.SERVER_PROTOCOL}://hcat.io:${process.env.SERVER_PORT}/user/verify_email?hash=` +
      passwordHash, // plain text body
    // html: '<b>Hello world?</b>' // html body
  };
...
router.get("/verify_email", function (req, res) {
  ...
  mysqlPoolQuery(
    "UPDATE user SET u_verify_status = 1 WHERE u_password_hash = '" +
      passwordHash +
      "'",
    function (err) {
      if (err) {
  ...
  switch (applicationType) {
    case "signUp":
      switch (pictureType) {
        case "u_imagef":
          mysqlPoolQuery(
            "UPDATE user SET u_imagef = ? WHERE u_email = ?",
            params,
            function (err) {
              if (err) {
                res.status(400).json({ message: "新增照片地址失敗" + err });
              } else {
                res.status(200).json({ message: "新增照片地址成功！" });
              }
            }
          );
          break;
        case "u_imageb":
          mysqlPoolQuery(
            "UPDATE user SET u_imageb = ? WHERE u_email = ?",
            params,
            function (err) {
              if (err) {
                res.status(400).json({ message: "新增照片地址失敗" + err });
        ...
        case "u_bankAccountimage":
          mysqlPoolQuery(
            "UPDATE user SET u_bankAccountimage = ? WHERE u_email = ?",
            params,
            function (err) {
              if (err) {
                res.status(400).json({ message: "新增照片地址失敗" + err });
    ...
    case "forget_password":
      switch (pictureType) {
        case "fp_imagef":
          mysqlPoolQuery(
            "UPDATE forget_pw SET fp_imagef = ? WHERE fp_investor_email = ?",
            params,
            function (err) {
              if (err) {
                res.status(400).json({ message: "新增照片地址失敗" + err });
        ...
        case "fp_imageb":
          mysqlPoolQuery(
            "UPDATE forget_pw SET fp_imageb = ? WHERE fp_investor_email = ?",
            params,
            function (err) {
              if (err) {
                res.status(400).json({ message: "新增照片地址失敗" + err });
        ...
        case "fp_bankAccountimage":
          mysqlPoolQuery(
            "UPDATE forget_pw SET fp_bankAccountimage = ? WHERE fp_investor_email = ?",
            params,
            function (err) {
              if (err) {
                res.status(400).json({ message: "新增照片地址失敗" + err });
  ...
  var qur = mysqlPoolQuery(qstr1, email, function (err, result) {
    if (err) {
      ...
      if (result.length === 0) {
        ...
        console.log(
          "[Success] 1 Email is found",
          result,
          "result[0]",
          result[0],
          "result[0].u_assetbookContractAddress",
          result[0].u_assetbookContractAddress
        );
        // var data = {
  ...
  if (token) {
    ...
    jwt.verify(token, "privatekey", function (err, decoded) {
      if (err) {
        ...
        //查詢Endorser Email
        let mysqlPoolQuery = req.pool;
        let query = mysqlPoolQuery(
          "SELECT u_endorser1,u_endorser2,u_endorser3 FROM  user WHERE u_email = ?",
          decoded.u_email,
          function (err, result) {
            if (err) {
  ...
  queryUserByEmail(EndorserEmail1)
    .then((results) => {
      ...
      if (results.length == 1 || results.length == 2) {
        ...
        let mysqlPoolQuery = req.pool;
        let query = mysqlPoolQuery(
          "UPDATE  user SET u_endorser1 = ? WHERE u_email = ?",
          [EndorserEmail1, userEmail],
          function (err) {
            if (err) {
    ...
    .then((results) => {
      ...
      if (results.length == 1 || results.length == 2) {
        ...
        let mysqlPoolQuery = req.pool;
        let query = mysqlPoolQuery(
          "UPDATE  user SET u_endorser2 = ? WHERE u_email = ?",
          [EndorserEmail2, userEmail],
          function (err) {
            if (err) {
    ...
    .then((results) => {
      ...
      if (results.length == 1 || results.length == 2) {
        ...
        let mysqlPoolQuery = req.pool;
        let query = mysqlPoolQuery(
          "UPDATE  user SET u_endorser3 = ? WHERE u_email = ?",
          [EndorserEmail3, userEmail],
          function (err) {
            if (err) {
...
router.post("/ForgetPassword", function (req, res, next) {
  ...
  mysqlPoolQuery(
    "SELECT * FROM  user WHERE u_email = ? AND u_identityNumber = ?",
    [email, nationalID],
    function (err, result) {
      /* 查詢失敗 */
      if (err) {
  ...
  query(isUserAbleToApplyQuery, email)
    .then((result) => {
      ...
      if (isUserAbleToApply) {
        mysqlPoolQuery(
          ...
          function (err, result) {
            ...
            if (err) {
              ...
              const tenMinutesAfterFP_application_date =
                getTenMinutesAfter_FP_application_date(
                  result[0].fp_application_date
                );
              /* 驗證碼已過期 */
        ...
        res
          .status(200)
          .json({ message: "您上筆申請的資料正在審閱中，請等待通知信" });
      }
    })
    .catch((err) => {
      res.status(400).send("查詢失敗：" + err);
...
router.get("/UpdateUserInformation", function (req, res, next) {
  ...
  query("UPDATE  user SET ? WHERE u_email = ?", [sql, email])
    .then(() => {
      return query("SELECT * FROM  user WHERE u_email = ?", email);
    })
    .then((result) => {
      const data = {
        u_email: result[0].u_email,
  ...
  bcrypt
    ...
    .then(() => {
      isForgetPassword
        ? res
            .status(200)
            .json({ message: "申請成功，請等待身份驗證通過後再以新密碼登入" })
        : res
            .status(200)
            .json({ message: "申請成功，請等待身份驗證通過後再重新登入" });
    })
    .catch((err) => {
      res.status(400).send("查詢失敗：" + err);
      console.error("query error : " + err);
  ...
  jwt.verify(JWT, process.env.JWT_PRIVATEKEY, async (err, decoded) => {
    if (err) {
      ...
      const getUserReviewStatusQuery = `
            ...
            WHERE  u_email = ?`;
      try {
        let userReviewStatus = await query(
          getUserReviewStatusQuery,
          decoded.u_email
        );
...
router.post("/AssetRecordDailySnapshot", function (req, res, next) {
  ...
  var qur = mysqlPoolQuery(
    "SELECT hi.ar_tokenSYMBOL,hu.u_name,hi.ar_investorEmail,hi.ar_Holding_Amount_in_the_end_of_Period,hu.u_bankcode,hu.u_bankBooklet FROM " +
      process.env.DB_NAME +
      ".investor_assetRecord hi," +
      process.env.DB_NAME +
      ".user hu where hu.u_email=hi.ar_investorEmail",
    async function (err, rows) {
      if (err) {
        ...
        var qur1 = mysqlPoolQuery(
          "INSERT INTO  AseetRecordDailySnapshot SET ?",
          sql,
          async function (err, rows) {
            if (err) {
              console.log(err);
              ...
              res
                .status(200)
                .json({ message: "寫入AssetRecordDailySnapshot成功" });
            }
          }
        );
      }
