...
router.get('/BackendUser_Platform_Supervisor', function (req, res, next) {
    ...
    if(JWT_decoded!==undefined){
        ...
        var iaData;
        mysqlPoolQuery("SELECT ia_SYMBOL,ia_time,ia_actualPaymentTime,ia_single_Forecasted_Payable_Income_in_the_Period,ia_single_Actual_Income_Payment_in_the_Period,ia_managementfee,ia_rent,ia_insurancepremium,ia_ElectricitySellingIncomeEachPiece FROM income_arrangement WHERE ia_State =?", "ia_state_underReview"  , function(err, rows) {
            if (err) {
                console.log(err);
            }
            iaData = rows;
