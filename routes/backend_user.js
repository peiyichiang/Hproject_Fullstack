  ...
  if (JWT_decoded !== undefined) {
    mysqlPoolQuery("SELECT * FROM backend_user", function (err, rows) {
      ...
      mysqlPoolQuery("SELECT * FROM user", function (err, rows) {
        ...
        mysqlPoolQuery(
          "SELECT fp_investor_email,fp_imagef,fp_imageb,fp_bankAccountimage,fp_application_date,fp_isApproved,fp_investor_account_status FROM forget_pw",
          function (err, rows) {
            if (err) {
              console.log(err);
            }
            var FP_data = rows;
            res.render("PlatformAdmin", {
              title: "Backend User Information",
...
router.post("/ForgetPassword", function (req, res, next) {
  ...
  mysqlPoolQuery(
    "SELECT * FROM backend_user WHERE m_id = ?",
    ID,
    function (err, rows) {
      if (err) {
        console.log(err);
      ...
      if (rows.length == 0) {
        ...
        let mailOptions = {
          ...
          text:
            ...
            "/BackendUser/ResetPassword?hash=" +
        ...
        transporter.sendMail(mailOptions, (err, info) => {
          if (err) {
            ...
            res.render("error", {
              message:
                '重新設置密碼連結 寄送成功,若信箱內沒找到信,若是信件夾找不到，有可能是被系統分類到"垃圾郵件"或"廣告郵件"裡，請到這幾個信件夾找尋認證信。',
              error: "",
            });
          }
...
router.post("/ResetPassword", function (req, res, next) {
  ...
  bcrypt
    ...
    .then((hash) => {
      ...
      var qur = mysqlPoolQuery(
        "UPDATE backend_user SET ? WHERE m_passwordhash = ?",
        [sql, ResetPasswordHash],
        function (err, rows) {
          if (err) {
            console.log(err);
...
router.get("/DeleteBackendUser", function (req, res, next) {
  ...
  var qur = mysqlPoolQuery(
    "UPDATE backend_user SET ? WHERE m_id = ?",
    [sql, ID],
    function (err, rows) {
      if (err) {
        console.log(err);
        ...
        bcrypt
          ...
          .then((compareResult) => {
            ...
            if (compareResult) {
              ...
              res.render("PasswordWrongMessage", {
                message: "密碼錯誤",
                error: "",
              });
            }
    ...
    mysqlPoolQuery(
      ...
      function (err, rows) {
        ...
        mysqlPoolQuery("SELECT * FROM user", function (err, rows) {
          ...
          //FrontEnd_data=前端使用者資料
          var FrontEnd_data = rows;
          res.render("PlatformCustomerService", {
            title: "Backend User Information",
            UserID: JWT_decoded.payload.m_id,
            ForgetPassword_data: data,
            FrontEnd_data: FrontEnd_data,
          });
        });
      }
...
router.post("/SetForgetPasswordApproved", function (req, res, next) {
  ...
  if (JWT_decoded !== undefined) {
    ...
    var qur = mysqlPoolQuery(
      "UPDATE forget_pw SET ? WHERE fp_investor_email = ? AND fp_application_date = ?",
      [sql, email, applicationTime],
      function (err, rows) {
        if (err) {
          console.log(err);
          // res.render('error', { message: '更改密碼失敗：' + err, error: '' });
          ...
          var qur = mysqlPoolQuery(
            "SELECT fp_salt,fp_password_hash FROM htoken.forget_pw WHERE fp_investor_email = ? AND fp_application_date = ?",
            [email, applicationTime],
            function (err, rows) {
              if (err) {
                console.log(err);
              } else {
                ...
                var qur = mysqlPoolQuery(
                  "UPDATE htoken.user SET ? WHERE u_email = ?",
                  [sql, email],
                  function (err, rows) {
                    if (err) {
                      console.log(err);
...
router.get("/BackendUser_Platform_Supervisor", function (req, res, next) {
  ...
  if (JWT_decoded !== undefined) {
    ...
    var iaData;
    mysqlPoolQuery(
      "SELECT ia_SYMBOL,ia_time,ia_actualPaymentTime,ia_single_Forecasted_Payable_Income_in_the_Period,ia_single_Actual_Income_Payment_in_the_Period,ia_managementfee,ia_rent,ia_insurancepremium,ia_ElectricitySellingIncomeEachPiece FROM income_arrangement WHERE ia_State =?",
      "ia_state_underReview",
      function (err, rows) {
        if (err) {
          console.log(err);
        }
