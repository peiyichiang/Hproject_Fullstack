...
router.post("/signUp", function (req, res) {
  ...
  mysqlPoolQuery(
    "SELECT u_email,u_verify_status,u_password_hash FROM user WHERE u_email = ?",
    [email],
    function (err, DBresult, rows) {
      if (
        DBresult.length == 0 ||
        (DBresult.length != 0 && DBresult[0].u_verify_status == 0)
      ) {
        console.log("the account are not verified yet\n");
        return res
          .status(404)
          .json({ success: "False", message: "信箱尚未驗證過" });
...
router.post("/verify_email", async function (req, res) {
  ...
  if (email && u_verify_code) {
    ...
    if (u_verify_code == verify_code && _time <= expr) {
      ...
      return res
        .status(200)
        .json({ success: "True", verify: true, message: "驗證成功" });
    }
    // else{
    ...
    return res
      ...
      .json({
        success: "False",
        message: "wrong or lack parameters + 驗證碼錯誤或是超過驗證時效",
      });
  }
  ...
  function getUserVerifyCode(mysqlPoolQuery, email) {
    return new Promise((resolve, reject) => {
      mysqlPoolQuery(
        "SELECT u_verify_code, u_verify_code_expr  FROM user WHERE u_email = ?",
        [email],
        function (err, DBresult, rows) {
          if (err) {
            console.log(err);
            ...
            resolve([
              DBresult[0].u_verify_code,
              DBresult[0].u_verify_code_expr,
            ]);
          }
        }
      );
...
router.post("/send_email", async function (req, res) {
  ...
  function addAccount(mysqlPoolQuery) {
    mysqlPoolQuery("INSERT INTO user SET ?", sql, function (err, result) {
      if (err) {
        ...
        return res
          .status(500)
          .json({ success: "False", message: "帳戶新增寫入資料庫失敗:\n" });
      } else {
    ...
    mysqlPoolQuery(
      ...
      function (err, result) {
        if (err) {
          ...
          return res
            .status(500)
            .json({ success: "False", message: "更新帳戶資料庫失敗\n" });
        } else {
  ...
  function checkUserAccount(mysqlPoolQuery, email) {
    mysqlPoolQuery(
      "SELECT u_email,u_verify_status  FROM user WHERE u_email = ?",
      [email],
      function (err, DBresult, rows) {
        if (err) {
          // sql wrong
          ...
          if (DBresult.length == 0) {
            // db can't find the email record
            console.log("add new account");
            addAccount(mysqlPoolQuery);
            sendEmail();
          } else {
            // the email is exist in db
            ...
            if (DBresult[0].u_verify_status == 0) {
              ...
              return res
                .status(404)
                .json({ success: "False", message: "帳戶已經驗證通過\n" });
            }
          }
        }
      }
